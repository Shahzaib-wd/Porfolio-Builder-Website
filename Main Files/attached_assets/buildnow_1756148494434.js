// Portfolio Builder JavaScript with Universal Template System
class PortfolioBuilder {
  constructor() {
    this.projects = [];
    this.maxProjects = 6;
    this.minProjects = 2;
    this.selectedTemplate = 1; // Default template
    this.currentStep = 'template'; // 'template' or 'form'
  ...
  loadExistingData() {
    ...
    if (existingData) {
      ...
      this.selectedTemplate = existingData.selectedTemplate || 1;
  ...
  initializeStep() {
    // Check if user has existing data, if so, show form step
    const existingData = this.getPortfolioData();
    if (existingData && existingData.personal && existingData.personal.name) {
      this.showFormStep();
    } else {
  ...
  setupEventListeners() {
    ...
    // Step navigation
    document.getElementById('continueWithTemplate').addEventListener('click', () => {
      this.showFormStep();
    });
  ...
  setupTemplateSelection() {
    const templateCards = document.querySelectorAll('.template-card:not(.coming-soon)');
    const continueButton = document.getElementById('continueWithTemplate');
    ...
    templateCards.forEach(card => {
      card.addEventListener('click', () => {
        ...
        this.selectedTemplate = parseInt(card.dataset.template);
    ...
    // Set initial selection if template is already selected
    if (this.selectedTemplate) {
      const selectedCard = document.querySelector(`[data-template="${this.selectedTemplate}"]:not(.coming-soon)`);
      if (selectedCard) {
  ...
  updateSelectedTemplateDisplay() {
    ...
    if (display) {
      display.textContent = `Template ${this.selectedTemplate}`;
    }
  }
  ...
  getPreviewUrl() {
    // Universal routing system for templates
    if (this.selectedTemplate === 1) {
      return 'preview.html'; // Template 1 uses preview.html
    } else {
      return `preview${this.selectedTemplate}.html`; // Template 2+ uses preview2.html, preview3.html, etc.
    }
  }
